...
class FFNode extends FFBase {
  ...
  remove(time, id) {
    ...
    TimelineUpdate.addFrameCallback(id, this.updateRemoveCallback);
  }
   ...
   */
  destroy() {
    const creator_id = this.root().id;
    TimelineUpdate.removeFrameCallback(creator_id, this.updateRemoveCallback);
